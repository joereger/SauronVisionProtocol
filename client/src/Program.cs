...
// Since we're having issues with MAUI dependencies, let's create a simple console client
// that can be used to test the protocol functionality
...
// Connection handler
async Task ConnectToServerAsync(ProtocolClient client)
{
...
// Disconnect handler
void DisconnectFromServer(ProtocolClient client)
{
    client.Disconnect();
...
async Task SendPalantirGazeCommandAsync(ProtocolClient client)
{
    ...
    if (int.TryParse(selection, out int index) && index >= 1 && index <= locations.Length)
    {
        ...
        {
            Console.WriteLine("\nResponse analysis:");
            var match = System.Text.RegularExpressions.Regex.Match(response, @"^(\d+)\s+(\w+)\s+""(.+)""$");
            if (match.Success)
            {
